...
function initializeDragAndDrop() {
    ...
    function showResolutionModal(conflict) {
        ...
        modalHTML += `
                ...
                <div class="suggestions-section">
                    ...
                    <div id="suggestion-loading">Analyzing timetable for the best options... <div class="spinner"></div></div>
                    <div id="suggestion-results" class="suggestion-results"></div>
                </div>
                <div class="move-to-section">
                    <h3>Or manually select a slot:</h3>
        ...
        const selectedBlock = conflict.blocks[0];
        const classGroup = selectedBlock.dataset.classGroup;
        const subject = selectedBlock.dataset.subject;
        const teacher = selectedBlock.dataset.teacher;
        
        fetchSuggestions(classGroup, subject, teacher, conflict).then(suggestions => {
            const suggestionResults = modal.querySelector('#suggestion-results');
            const loadingElement = modal.querySelector('#suggestion-loading');
            
            loadingElement.style.display = 'none';
            
            if (suggestions.length === 0) {
                suggestionResults.innerHTML = '<p class="no-suggestions">No optimal suggestions available. Please select a slot manually.</p>';
            } else {
                // Display the top suggestions
                let suggestionsHTML = '<div class="suggestion-list">';
                
                suggestions.forEach((suggestion, index) => {
                    const { day, start_time, end_time, room, score, reason } = suggestion;
                    const timeSlot = `${start_time}-${end_time}`;
                    
                    suggestionsHTML += `
                        <div class="suggestion-item ${index === 0 ? 'best-suggestion' : ''}">
                            <div class="suggestion-header">
                                <div class="suggestion-title">
                                    <span class="day-badge">${day}</span>
                                    <span class="time-badge">${timeSlot}</span>
                                    <span class="room-badge">Room ${room}</span>
                                </div>
                                <div class="suggestion-score">
                                    <span class="score-value" title="Suggestion quality score">
                                        ${getScoreStars(score)}
                                    </span>
                                </div>
                            </div>
                            <div class="suggestion-reason">${reason}</div>
                            <button class="btn apply-suggestion" data-day="${day}" data-time="${timeSlot}" data-room="${room}">
                ...
                suggestionsHTML += '</div>';
                suggestionResults.innerHTML = suggestionsHTML;
                
                // Add event listeners for suggestion buttons
                suggestionResults.querySelectorAll('.apply-suggestion').forEach(button => {
                    button.addEventListener('click', function() {
    ...
    // Helper function to get score stars based on score value (1-10)
    function getScoreStars(score) {
        const normalizedScore = Math.min(Math.max(Math.round(score * 5 / 10), 1), 5); // Convert to 1-5 scale
        const stars = '★'.repeat(normalizedScore) + '☆'.repeat(5 - normalizedScore);
        return stars;
    }
    
    // Function to fetch suggestions from the server
    async function fetchSuggestions(classGroup, subject, teacher, conflict) {
        try {
            // Get selected block
            const selectedBlockIndex = parseInt(document.querySelector('input[name="conflict-resolve"]:checked')?.value || "0");
            const selectedBlock = conflict.blocks[selectedBlockIndex];
            ...
            const data = {
                ...
                subject: subject,
                teacher: teacher,
                room: selectedBlock.dataset.room,
                day: selectedBlock.parentElement.dataset.day,
                time_slot: selectedBlock.parentElement.dataset.time,
                conflict_type: conflict.type
            };
            
            // Send request to get suggestions
            const response = await fetch('/suggest_timetable_slots', {
                method: 'POST',
                headers: {
