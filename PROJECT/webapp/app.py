...
import collections
import os
# Use relative path with explicit path manipulation
        ...
        if 'csv_file' in request.files:
            ...
            if file.filename.endswith('.csv'):
                ...
                for row in reader:
                    ...
                    try:
                        weekly_hours = int(weekly_hours) if weekly_hours else 8
                    except ValueError:
                        weekly_hours = 8
                    if name and subject:
                        add_teacher(session, name, subject, weekly_hours)
                return redirect(url_for('index'))
        # fallback for manual (should not happen)
        name = request.form.get('name')
...
def edit_teacher(teacher_id):
    ...
    if request.method == 'POST':
        ...
        if name and subject:
            ...
            teacher.weekly_hours = weekly_hours
...
def teacher_class_counts():
    ...
    # Get all teachers and their weekly hour limits
    teachers = session.query(Teacher).all()
    for teacher in teachers:
        teacher_limits[teacher.id] = teacher.weekly_hours
        teacher_names[teacher.id] = teacher.name
    
    if active_timetable:
        # Method 1: Count directly from Timetable entries
        for teacher_id, weekly_limit in teacher_limits.items():
            count = session.query(Timetable).filter_by(teacher_id=teacher_id).count()
            teacher_counts[teacher_names[teacher_id]] = count
            
        # If no entries found, try analyzing the JSON data
        if not any(teacher_counts.values()):
            # Load timetable data
    ...
    if not teacher_counts:
        ...
        teacher_limits = {"Jane Smith": 8, "John Doe": 6, "Alice Johnson": 10, "Robert Brown": 8}
    
    # Create data for chart
    chart_data = []
    for teacher_name, count in teacher_counts.items():
        # Find the teacher's limit
        limit = 8  # Default
        for teacher in teachers:
            if teacher.name == teacher_name:
                limit = teacher.weekly_hours
                break
    ...
    # Sort by scheduled hours (highest first)
    chart_data.sort(key=lambda x: x['scheduled'], reverse=True)
    
    # Format data for chart
    labels = [item['name'] for item in chart_data]
    scheduled = [item['scheduled'] for item in chart_data]
    limits = [item['limit'] for item in chart_data]
    remaining = [item['remaining'] for item in chart_data]
    over_limit = [item['overLimit'] for item in chart_data]
...
def teacher_timetable(teacher_id):
    ...
    # Calculate current scheduled hours
    current_hours = session.query(Timetable).filter_by(teacher_id=teacher_id).count()
...
@app.route('/suggest_timetable_slots', methods=['POST'])
def suggest_timetable_slots():
    """Generate intelligent suggestions for resolving timetable conflicts."""
    try:
        data = request.json
        if not data:
            return jsonify({'success': False, 'error': 'No data provided'}), 400
        ...
        # Get the class ID and course ID for the suggest_reschedule_options function
        class_obj = session.query(Class).filter_by(name=class_group).first()
        course_obj = session.query(Course).filter_by(name=subject).first()
        
        if not class_obj or not course_obj:
            return jsonify({'success': False, 'error': 'Class or course not found'}), 404
            
        # Get suggestions from the scheduler
        suggestions = suggest_reschedule_options(session, class_obj.id, course_obj.id)
        
        # Convert suggestions to a more useful format with reasoning
        formatted_suggestions = []
        
        for day, start_time, end_time, classroom in suggestions:
            time_slot = f"{start_time}-{end_time}"
            
            # Analyze the suggestion to provide a reason and score
            score, reason = analyze_suggestion(day, start_time, end_time, classroom, class_group, teacher)
        ...
        formatted_suggestions.sort(key=lambda x: x['score'], reverse=True)
        
        # Limit to top 5 suggestions
        top_suggestions = formatted_suggestions[:5]
        ...
        print(f"Error generating timetable suggestions: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500

def analyze_suggestion(day, start_time, end_time, classroom, class_group, teacher):
    """Analyze a suggestion and provide a reason and score (1-10)."""
    try:
        # Start with a base score
        score = 8.0
        reasons = []
        
        # Check the day distribution (prefer to balance days)
        class_obj = session.query(Class).filter_by(name=class_group).first()
        if class_obj:
            # Count classes per day for this class group
            day_counts = {}
            for timetable in session.query(Timetable).filter_by(class_id=class_obj.id).all():
                day_counts[timetable.day] = day_counts.get(timetable.day, 0) + 1
            
            # Check if this day already has many classes
            current_count = day_counts.get(day, 0)
            if current_count >= 3:
                score -= 1.5
                reasons.append(f"{day} already has {current_count} classes for {class_group}")
            elif current_count <= 1:
                score += 1
                reasons.append(f"{day} is less utilized for {class_group}")
        
        # Check teacher workload on this day
        teacher_obj = session.query(Teacher).filter_by(name=teacher).first()
        if teacher_obj:
            # Count classes per day for this teacher
            day_counts = {}
            for timetable in session.query(Timetable).filter_by(teacher_id=teacher_obj.id).all():
                day_counts[timetable.day] = day_counts.get(timetable.day, 0) + 1
            ...
            if current_count >= 3:
                score -= 1
                reasons.append(f"{teacher} already teaches {current_count} classes on {day}")
            elif current_count == 0:
                score += 1.5
                reasons.append(f"{teacher} has no other classes on {day}")
        
        # Check time distribution (prefer to avoid early morning for lab sessions)
        # Get course information from the database based on class and teacher
        course_info = session.query(ClassCourseTeacher).join(Course).filter(
            ClassCourseTeacher.class_id == class_obj.id if class_obj else 0,
            ClassCourseTeacher.teacher_id == teacher_obj.id if teacher_obj else 0
        ).first()
        
        if course_info and course_info.course:
            course_name = course_info.course.name.lower()
            if 'lab' in course_name and start_time == "08:00":
                score -= 1
                reasons.append(f"Early morning lab sessions are not ideal")
        
        # Prefer larger rooms for lab sessions
        classroom_obj = session.query(Classroom).filter_by(name=classroom).first()
        
        # Use the course_info from previous check
        if course_info and course_info.course and classroom_obj:
            course_name = course_info.course.name.lower()
            if 'lab' in course_name:
                if classroom_obj.capacity > 30:
                    score += 1
                    reasons.append(f"Room {classroom} has good capacity for lab sessions")
                else:
                    score -= 0.5
                    reasons.append(f"Room {classroom} may be small for lab sessions")
        
        # Adjust score based on specific times
        time_hour = int(start_time.split(':')[0])
        if time_hour < 10:
            score -= 0.5
            reasons.append("Early morning classes may have lower attendance")
        elif 11 <= time_hour <= 14:
            score += 1
            reasons.append("Mid-day slot is optimal for student engagement")
        elif time_hour >= 16:
            score -= 0.5
            reasons.append("Late afternoon classes may face student fatigue")
        
        # Ensure score is within range
        score = min(max(score, 1), 10)
        
        # Select the top 2-3 reasons for readability
        if len(reasons) > 2:
            reasons = reasons[:2]
        ...
        print(f"Error analyzing suggestion: {e}")
        return 5, "Standard available slot."
