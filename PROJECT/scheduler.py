...
# Ensure all models are imported so all tables are created
from models import ClassCancellation, RoomChange, ApprovedTimetable, Event, Feedback
...
class Teacher(Base):
    ...
    def __repr__(self):
        return f"<Teacher(name={self.name}, subject={self.subject}, weekly_hours={self.weekly_hours})"
...
def add_teacher(session, name, subject, weekly_hours=8):
    teacher = Teacher(name=name, subject=subject, weekly_hours=weekly_hours)
    session.add(teacher)
    session.commit()
    return teacher
...
def generate_timetable(session, days, time_slots):
    ...
    Respects teacher's weekly working hour limits.
    ...
    # Helper: check if slot is free for all resources and teacher has hours available
    def free_for_all(day, start, end, room_id, teacher_id, class_id):
        # Get teacher and check if adding 1 hour would exceed their weekly limit
        teacher = next((t for t in teachers if t.id == teacher_id), None)
        if teacher and (teacher_hours[teacher_id] + 1 > teacher.weekly_hours):
            return False
    ...
    for class_ in classes_list:
        ...
        for cct in ccts:
            ...
            # Skip if teacher has reached their weekly hours limit
            if teacher_hours[teacher.id] >= teacher.weekly_hours:
                summary.append(f"⚠️ Could not schedule {course.name} for {class_.name} - {teacher.name} has reached {teacher.weekly_hours} hour limit")
                continue
            ...
            # Skip lab if teacher doesn't have at least 2 hours left
            if teacher_hours[teacher.id] + 2 <= teacher.weekly_hours:
                for day in lab_days:
            ...
            # 2) Schedule LECTURES (single slots), shuffle days for even spread
            # Calculate how many hours are left for this teacher
            remaining_teacher_hours = teacher.weekly_hours - teacher_hours[teacher.id]
            # Default to 3 lectures, but cap at remaining hours
            lectures_needed = min(3, remaining_teacher_hours)
            
            if lectures_needed <= 0:
                summary.append(f"⚠️ Could not schedule lectures for {course.name} in {class_.name} - {teacher.name} has reached {teacher.weekly_hours} hour limit")
                continue
    ...
    # Add summary of teacher hours
    for teacher in teachers:
        hours_used = teacher_hours[teacher.id]
        summary.append(f"Teacher {teacher.name}: {hours_used}/{teacher.weekly_hours} hours used")
    
    print("Timetable generation complete respecting teacher weekly hours.")
    return summary
...
def find_available_rooms(session, day, start_time, end_time):
    """
    Returns a list of available classrooms for the given day and time range.
    Checks that rooms are free for the entire duration between start_time and end_time,
    not just at the exact time slot.
    Handles various time formats (24 hour or 12 hour).
    """
    # Format time to HH:MM if it's in a different format
    # HTML time input could send format like '13:45'
    # Convert to the format stored in database (like '13:45')
    if start_time and ':' in start_time:
        # Already in right format, no need to change
        formatted_start = start_time
    else:
        # Default format if not properly formatted
        formatted_start = start_time
    ...
    print(f"DEBUG: Searching for rooms free between {day} {formatted_start}-{formatted_end}")
        
    # Get all rooms
    all_rooms = session.query(Classroom).all()
    
    # Get all timetable entries for this day that might overlap with our requested time range
    timetable_entries = session.query(Timetable).filter_by(day=day).all()
    ...
    requested_start_minutes = time_to_minutes(formatted_start)
    requested_end_minutes = time_to_minutes(formatted_end)
    
    # Find rooms that have any overlapping bookings in the requested time range
    occupied_ids = set()
    
    for entry in timetable_entries:
        entry_start_minutes = time_to_minutes(entry.start_time)
        entry_end_minutes = time_to_minutes(entry.end_time)
        
        # Check if there is any overlap between the requested time range and this entry
        # An overlap exists if the entry starts before the requested period ends,
        # and ends after the requested period starts
        if (entry_start_minutes < requested_end_minutes and 
            entry_end_minutes > requested_start_minutes):
            occupied_ids.add(entry.classroom_id)
    
    print(f"DEBUG: Found {len(occupied_ids)} occupied rooms in the time range: {occupied_ids}")
    
    # Filter to only available rooms (those not in the occupied set)
...
if __name__ == "__main__":
    ...
    from models import ApprovedTimetable
    import json
